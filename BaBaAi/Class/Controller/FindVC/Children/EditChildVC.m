...
@interface EditChildVC ()<UIGestureRecognizerDelegate,AddChildrenCellDelegate,GenderPickerViewDelegate,BirthdayPickerViewDelegate,UINavigationBarDelegate,UITextFieldDelegate,UIImagePickerControllerDelegate,ITcpClient>{
...
-(void)showBirthPicker:(NSIndexPath *)indexPath{
    ...
    bodView.defaultBirdOfDate = birthOfDate;
    NSLog(@"defaultBirdOfDate:%@",bodView.defaultBirdOfDate);
...
-(void)loadCustomView{
    ...
    [self rightNavBarItemWithTitle:MLString(@"保存") AndSel:@selector(submitEditChild:)];
    ...
    childLogo.image = [[FileManagerHelper share] getImageViewFromImage:_cModel.llWearId];
    ...
    if (_cModel.sbirthdate!=nil) {
        ...
        //被注释
//        birthOfDate      = [NSString changePureNumberToDobWith:birth];
    ...
    shipIndex = (_cModel.iRelationship==0)?1:_cModel.iRelationship;
    //获得性别
    genderIndex = _cModel.iGender;
 ...
 * 提交保存小孩信息
 */
-(void)submitEditChild:(UIBarButtonItem *)sender
{
    if ([self checkChildInput]) {
        

        _cModel.simei = imeiString;
        _cModel.sChildName   = sChildName;
        _cModel.iGender      = genderIndex;
        ...
        //被注释
        _cModel.sbirthdate   = [NSString stringWithString:birthOfDate];
        _cModel.iHeight      = [sHeight intValue];
        _cModel.fWeight      = [sWeight floatValue];
        _cModel.iRelationship = shipIndex;
        _cModel.iRelationshipPic = shipIndex;
        _cModel.sPhone       = sPhone;
        _cModel.sWatchphone  = sWatchphone;
        
        
        NSDictionary *wearer  = @{@"childrenID":[NSNumber numberWithUnsignedLongLong:_cModel.llWearId],
                                  @"imei":imeiString,
                                  ...
                                  @"birthDay":[NSString stringWithString:birthOfDate],
        ...
        NSMutableDictionary *mutableDictionary = [[NSMutableDictionary alloc] initWithDictionary:wearer];
        TcpClient *tcp = [TcpClient sharedInstance];
        [tcp setDelegate_ITcpClient:self];
        
        
        //写入数据库
        [[TBChild shareDB] insert:_cModel];
...
//            dataPacket.sCommand       = @"U_CHILDREN";
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    _cModel.selectPicture = selectPicture;
    ...
    _cModel.childLogoData = uploadImageData;
    //上传图片
    [self uploadChildLogo:uploadImageData];
...
-(void)OnReciveData:(NSDictionary *)recivedTxt{
    ...
    if ([self checkSocketRespClass:recivedTxt]){
        ...
        if ([self getCommod]!=nil) {
            ...
            }else if ([[self getCommod] isEqualToString:@"R_U_CHILDREN"] == YES) {
 ...
 * 上传图片
 */
-(void)uploadChildLogo:(NSData *)imgData{
    
    if (imgData!=nil) {
        //被注释
//    _cModel.sPicturePath = [NSData  writeToChildLogo:imgData withiWearId:_cModel.llWearId];
    _cModel.iChildLogoLenght = imgData.length;
    _cModel.sFileType = @"jpg";
    //执行生产图片命令
    NSDictionary *para  = @{@"childrenID":[NSNumber numberWithLongLong:_cModel.llWearId],@"fileType":_cModel.sFileType};
...
//        dataPacket.paraDictionary = mutableDictionary;
//        dataPacket.iType          = 1;
//        dataPacket.dataContent    = imgData;
//        [tcp sendContent:dataPacket];
//        
//    }else{
//        //[SVProgressHUD showErrorWithStatus:MLString(@"当前网络异常，请检查网络设置后重试！")];
